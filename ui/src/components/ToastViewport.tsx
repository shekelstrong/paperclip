...
function AnimatedToast({
  ...
  return (
    ...
    >
      <div classИмя="flex items-start gap-3 px-3 py-2.5">
        <span classИмя={cn("mt-1 h-2 w-2 shrink-0 rounded-full", toneDotClasses[toast.tone])} />
        <div classИмя="min-w-0 flex-1">
          <p classИмя="text-sm font-semibold leading-5">{toast.title}</p>
          {toast.body && (
            <p classИмя="mt-1 text-xs leading-4 opacity-70">
              {toast.body}
            </p>
          )}
          {toast.action && (
            <Link
              ...
              onClick={() => onЗакрыть(toast.id)}
              classИмя="mt-2 inline-flex text-xs font-medium underline underline-offset-4 hover:opacity-90"
            >
              {toast.action.label}
            </Link>
          )}
        ...
        <button
          type="button"
          aria-label="Закрыть notification"
          onClick={() => onЗакрыть(toast.id)}
          classИмя="mt-0.5 shrink-0 rounded p-1 opacity-50 hover:bg-black/10 hover:opacity-100 dark:hover:bg-white/10"
        >
...
export function ToastViewport() {
  ...
  return (
    <aside
      aria-live="polite"
      aria-atomic="false"
      classИмя="pointer-events-none fixed bottom-3 left-3 z-[120] w-full max-w-sm px-1"
    >
      <ol classИмя="flex w-full flex-col-reverse gap-2">
        {toasts.map((toast) => (
          <AnimatedToast
            key={toast.id}
            toast={toast}
            onЗакрыть={dismissToast}
          />
        ))}
      </ol>
    </aside>
