...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? "Очистить репозиторий из этого рабочего пространства?"
        : "Удалить репозиторий этого рабочего пространства?",
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        ...
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label="Целевая дата" state="idle" />}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  aria-label="Помощь по кодовой базе"
                >
                  ?
                </button>
              </TooltipTrigger>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  >
                    {codebase.localFolder ? "Сменить папку" : "Задать папку"}
                  </Button>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                      aria-label="Справка по рабочим пространствам выполнения"
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? "Скрыть расширенные настройки checkout"
                          : "Показать расширенные настройки checkout"}
                      </button>
                    </div>
