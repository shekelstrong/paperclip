...
async function inflateZipEntry(compressionMethod: number, bytes: Uint8Array) {
  ...
  if (compressionMethod !== 8) {
    throw new Error("Неподдерживаемый zip-архив: поддерживаются только STORE и DEFLATE записи.");
  }
...
export async function readZipArchive(source: ArrayBuffer | Uint8Array): Promise<{
  ...
  while (offset + 4 <= bytes.length) {
    ...
    if (signature !== 0x04034b50) {
      throw new Error("Некорректный zip-архив: неподдерживаемый local file header.");
    }

    if (offset + 30 > bytes.length) {
      throw new Error("Некорректный zip-архив: усечённый local file header.");
    }
    ...
    if ((generalPurposeFlag & 0x0008) !== 0) {
      throw new Error("Неподдерживаемый zip-архив: data descriptors не поддерживаются.");
    }
    ...
    if (bodyEnd > bytes.length) {
      throw new Error("Некорректный zip-архив: усечённое содержимое файла.");
    }
