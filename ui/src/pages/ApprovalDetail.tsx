...
export function ApprovalDetail() {
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Комментарий не удался"),
  });
    ...
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("Удалить этого неутверждённого агента? Это нельзя отменить.")) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? "Posting…" : "Опубликовать комментарий"}
          </Button>
        </div>
      </div>
    </div>
