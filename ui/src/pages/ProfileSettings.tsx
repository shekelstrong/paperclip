...
export function ProfileSettings() {
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error("Выберите компанию перед загрузкой аватара профиля.");
      }
  ...
  const uploadHint = selectedCompany
    ...
    : "Выберите компанию для загрузки аватара в хранилище Paperclip.";
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? "Сменить фото" : "Загрузить фото"}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? "Нет email"}</p>
                </div>
        ...
        >
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? "Saving..." : "Сохранить профиль"}
            </Button>
          </div>
        </form>
      </section>
